...
export default function TutoringCommunicationPage() {
  ...
  const selectedThread = useMemo(
    () => tutoringThreads.find((thread) => thread.id === selectedThreadId) ?? tutoringThreads[0],
    [selectedThreadId],
  )
    ...
    <div className="grid gap-4 xl:grid-cols-[1.05fr_.95fr]">
      <section className="space-y-4">
        <div className="rounded-[1.5rem] border border-slate-900/10 bg-white p-6 shadow-sm">
          ...
          <h1 className="font-display mt-4 text-3xl tracking-tight text-slate-950">Threads that need a reply</h1>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-600">
            Keep parent, tutor, and student conversations in one place. Active view: {activePov}.
          </p>
        </div>

        <div className="space-y-3">
          {tutoringThreads.map((thread) => (
            <button
              key={thread.id}
              type="button"
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-500">{thread.channel} · {thread.subject}</p>
                </div>
      ...
      <aside className="space-y-4">
        <div className="rounded-[1.25rem] border border-slate-900/10 bg-white p-5 shadow-sm">
          {selectedThread ? (
            <div className="space-y-4">
              <div>
                <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Selected thread</div>
                <p className="mt-2 text-lg font-semibold text-slate-950">{selectedThread.studentName}</p>
                <p className="text-sm text-slate-500">{selectedThread.channel} · {selectedThread.subject}</p>
              </div>
              ...
              <label className="block">
                <span className="mb-2 block text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Draft reply</span>
                <textarea
                  value={draft}
                  onChange={(event) => setDraft(event.target.value)}
                  rows={5}
                  placeholder="Write a concise reply for the parent or student..."
              ...
              <div className="flex flex-wrap gap-2">
                {['Thanks for the update', 'I’ll send the recap tonight', 'Let’s review this in the next session'].map((snippet) => (
        ...
        <div className="rounded-[1.25rem] border border-slate-900/10 bg-slate-950 p-5 text-white shadow-lg shadow-slate-950/10">
          <div className="text-xs font-semibold uppercase tracking-[0.18em] text-[#f2c06d]">Queue summary</div>
          <div className="mt-4 grid grid-cols-3 gap-3 text-center">
            {[
              ['Unread', tutoringThreads.filter((thread) => thread.unread).length],
              ['Parent', tutoringThreads.filter((thread) => thread.channel === 'Parent').length],
              ['Need reply', tutoringThreads.length],
            ].map(([label, value]) => (
          ...
          <p className="mt-4 text-sm leading-7 text-white/75">
            Replies are local-only in this MVP, but the route is real so each sidebar action can own its own workflow page.
          </p>
        </div>
      </aside>
    </div>
