...
import { initialStudents, nextSessionKey, progressLabel, tutoringSectionMeta } from './tutoring-data'
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h1 className="font-display mt-4 text-4xl tracking-tight text-slate-950 sm:text-5xl">
              {tutoringSectionMeta.dashboard.title}
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600">
              Route-backed sections now split Students, Recaps, Progress, Schedule, Communication, and Settings into separate pages. Active view: {activePov}.
            </p>
          </div>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.08fr_.92fr]">
          ...
          <div className="mt-5 space-y-3">
            {(priorityStudents.length ? priorityStudents : upcoming.slice(0, 3)).map((student, index) => (
              <div key={student.id} className="rounded-2xl border border-slate-900/10 bg-[#f8f5ef] p-4">
                ...
                <p className="mt-3 text-sm leading-7 text-slate-600">{student.note}</p>
              </div>
            ))}
          </div>
        </article>
        ...
        <article className="rounded-[1.5rem] border border-slate-900/10 bg-slate-950 p-6 text-white shadow-lg shadow-slate-950/10">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#f2c06d]">Upcoming</p>
          <h2 className="mt-2 text-2xl font-semibold">Next sessions and timing</h2>
      ...
      <section>
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-slate-950">Jump into the right page</h2>
          </div>
        </div>
