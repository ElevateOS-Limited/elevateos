...
import { initialStudents, nextSessionKey } from './tutoring-data'
    ...
    <div className="grid gap-4 xl:grid-cols-[1.05fr_.95fr]">
      <section className="space-y-4">
        <div className="rounded-[1.5rem] border border-slate-900/10 bg-white p-6 shadow-sm">
          ...
          <h1 className="font-display mt-4 text-3xl tracking-tight text-slate-950">Weekly availability and session timing</h1>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-600">
            Manage open days, blocked dates, and upcoming tutoring slots without leaving the dashboard. Active view: {activePov}.
          </p>
          ...
          <div className="mt-4 space-y-3">
            {upcoming.map((student) => (
              <div key={student.id} className="rounded-[1rem] border border-slate-900/10 bg-[#f8f5ef] p-4">
                ...
                <p className="mt-2 text-sm leading-7 text-slate-600">{student.progress}% progress · {student.note}</p>
              </div>
            ))}
          </div>
        </div>
