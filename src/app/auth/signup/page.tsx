...
export default function SignupPage() {
  ...
  if (!signupEnabled) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 px-4">
        <div className="max-w-md text-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-8">
          <h1 className="text-2xl font-bold">Signup Disabled</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">Accounts are created by admin only. Please use your assigned login credentials.</p>
          <Link href="/auth/login" className="inline-block mt-4 text-indigo-500 hover:text-indigo-600 font-medium">Go to Login</Link>
        </div>
      </div>
    )
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const referralSource = params.get('from') || undefined
      const referralStudySessionId = params.get('sid') || undefined
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-indigo-500 mt-2">AoK tutors can activate Tutoring Premium access for free with an invite code.</p>
        </div>
          ...
          {googleAvailable && (
            <>
              <button onClick={() => signIn('google', { callbackUrl: '/dashboard' })}
                className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} required
                className="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
            ...
            <div className="rounded-xl border border-gray-300 dark:border-gray-700 p-3 space-y-3">
              <label className="flex items-center gap-2 text-sm">
                ...
                I am an AoK tutor (free Tutoring Premium access)
              </label>
              {isAokTutor && (
                <input
                  type="text"
